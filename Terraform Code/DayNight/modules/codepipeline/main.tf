    ...
    Statement = concat(
      [
        ...
        },
        {
          ...
          Resource = var.test_lambda_function_arn
        },
        {
...
# Wait for IAM eventual consistency before creating CodePipeline.
...
resource "aws_codepipeline" "this" {
  ...
  # Ensure IAM role/policy propagation completes before creating pipeline.
  ...
  # Stage 2: call Lambda to validate artifact before deployment.
  stage {
    name = "Test"
    ...
    action {
      ...
      provider = "Lambda"
      version  = "1"
      # Pass source artifact to Lambda so it can validate ZIP content.
      input_artifacts = ["source_output"]

      configuration = {
        FunctionName = var.test_lambda_function_name
      }
    }
  }

  # Stage 3: deploy built artifact to S3 website bucket.
  stage {
    name = "DeployToS3"
  ...
  # Stage 4: call Lambda to invalidate CloudFront cache.
  stage {
